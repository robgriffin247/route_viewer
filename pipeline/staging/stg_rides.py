...
def stg_rides():
    ...
    for file in files:
        world, route = [
            f.replace("_", " ").replace(".gpx", "") for f in file.split("__")
        ]

        i += 1
        print(f"{i} of {len(files)}: {world}, {route}")
        ...
        if route_id not in route_ids:
            ...
            row_data["distance_delta"] = [0] + [
                geopy.distance.distance(from_, to).m
    ...
    with duckdb.connect(
        ...
        con.sql(
            f'CREATE OR REPLACE TABLE {data_config["schema_stg"]}.{inspect.stack()[0][3]} AS SELECT file, world, route, longitude, latitude, altitude, distance FROM df'
        )
