...
if "user_notes" not in st.session_state:
    st.session_state["user_notes"] = pd.DataFrame({
        "highlight":[bool()], 
        "segment":[None], 
        "start":[None], 
        "end":[None], 
...
#input_platform, input_world, input_route, input_metric = st.columns([4,4,8,4], vertical_alignment="bottom")
input_world, input_route, input_metric = st.columns([4,8,4], vertical_alignment="bottom")

metric = input_metric.toggle("Metric", value=True)
...
with duckdb.connect("data/data.duckdb") as con:
    # User input to select platform
    #platforms = con.sql(f"""SELECT PLATFORM FROM INTERMEDIATE.OBT_FIT""").to_df()
    #platform = input_platform.selectbox("Platform", platforms.platform.unique(), index=len(list(platforms.platform.unique()))-1)
    platform = "Zwift"

    # User input to select world
    worlds = con.sql(f"""SELECT WORLD FROM INTERMEDIATE.OBT_FIT WHERE PLATFORM='{platform}'""").to_df()
    world = input_world.selectbox("World", worlds.world.unique(), index=len(list(worlds.world.unique()))-1)

    # User input to select route (as a child of world)
    routes = con.sql(f"""SELECT ROUTE FROM INTERMEDIATE.OBT_FIT WHERE PLATFORM='{platform}' AND WORLD='{world}'""").to_df()
    route = input_route.selectbox("Route", routes.route.unique())

    # Get the route data
    route_data = con.sql(f"""WITH route AS (
                            SELECT ROW_NUMBER() OVER () AS row, *, 
                            FROM INTERMEDIATE.OBT_FIT 
                            WHERE PLATFORM='{platform}' AND WORLD='{world}' AND ROUTE='{route}'
                        ),
...
if add_custom:
    ...
    base_notes = pd.concat([custom_notes, base_notes])
    ...
    race_notes = st.data_editor(base_notes,
                                column_config={
                                    "highlight":st.column_config.CheckboxColumn("Highlight", default=True),
                                    "segment":st.column_config.TextColumn("Segment"),
                                    "start":st.column_config.NumberColumn("From", format="%.1f"),
